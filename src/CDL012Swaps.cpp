...
FitResult CDL012Swaps::Fit()
{
    ...
    for (unsigned int t = 0; t < MaxNumSwaps; ++t)
    {
        ...
        for (auto& i : NnzIndices)
        {
            ...
            if (0.5*arma::dot(r,r) + ModelParams[0] + ModelParams[1]*B[i] + ModelParams[2]*B[i]*B[i] > 0.5*arma::dot(ri,ri) - riX[maxindex]*betaj + 0.5*betaj*betaj + ModelParams[0] + ModelParams[1]*std::fabs(betaj) + ModelParams[2]*betaj*betaj)
            {
